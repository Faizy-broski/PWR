-- Atomically allocates `p_quantity` unique, sequential ticket numbers for a
-- paid transaction and creates the resulting entry. Runs inside a single
-- statement-level transaction; the `for update` lock on the competition row
-- serializes concurrent purchases of the same competition so ticket numbers
-- can never be duplicated or oversold.
--
-- security definer so it can write entries/tickets even though the RLS
-- policies on those tables otherwise only allow admin writes (see
-- 20260807000003_rls_policies.sql) — regular purchases are expected to go
-- through this function rather than direct inserts.
...
begin
  ...
  select id, ticket_numbers into v_entry_id, v_numbers
  from public.entries
...
grant execute on function public.purchase_entry(uuid, uuid, integer, boolean) to authenticated, service_role;
